...
Redis 7.4부터는 Hash 내부의 각 field에도 TTL을 설정할 수 있습니다. 예전에는 Hash 전체 key에는 만료 시간을 줄 수 있었지만, 개별 field에는 줄 수 없었습니다.

예를 들어 세션을 아래처럼 하나의 Hash에 모아둘 수 있습니다.

```shell
HSET active:sessions session:1 "{...}"
HSET active:sessions session:2 "{...}"
```

문제는 field마다 성격이 다를 수 있다는 점입니다. `session:1`과 `session:2`의 TTL을 다르게 두고 싶어도, Redis 7.4 이전에는 field 단위 만료를 지원하지 않았습니다. 그래서 아래처럼 `EXPIRE`를 걸면 특정 field가 아니라 `active:sessions` 전체가 함께 만료됩니다.

```shell
EXPIRE active:sessions 1800
```

그래서 Redis 7.4 이전에는 이 문제를 보통 두 가지 방향으로 우회했습니다.

- field를 별도 key로 분리하고 일반 `EXPIRE`를 사용
- Hash는 유지하되 만료 시각을 애플리케이션에서 따로 관리

이 글은 바로 이 차이를 기준으로 정리합니다. Redis 7.4 이전에는 왜 우회가 필요했는지, 7.4 이후에는 `HEXPIRE` 계열 명령으로 무엇이 단순해졌는지, 그리고 Spring Data Redis에서 어디까지 바로 사용할 수 있는지를 순서대로 살펴보겠습니다.
...
```shell
SET active:session:session:1 "{...}" EX 1800
SET active:session:session:2 "{...}" EX 600
SET active:session:session:3 "{...}" EX 3600
```
...
class SessionCache(
    ...
    fun put(sessionId: String, payload: String, ttl: Duration) {
        val key = "active:session:$sessionId"
...
```shell
HSET active:sessions session:1 "{...}"
ZADD active:sessions:expire-at 1714719600000 session:1
```
...
```kotlin
@Service
class SessionExpireJob(
    private val redisTemplate: StringRedisTemplate,
) {
    companion object {
        private const val HASH_KEY = "active:sessions"
        private const val EXPIRE_INDEX_KEY = "active:sessions:expire-at"
    }

    fun put(sessionId: String, payload: String, expireAt: Instant) {
        redisTemplate.opsForHash<String, String>().put(HASH_KEY, sessionId, payload)

        // 만료 시각을 score로 저장한다.
        redisTemplate.opsForZSet()
            .add(EXPIRE_INDEX_KEY, sessionId, expireAt.toEpochMilli().toDouble())
    }

    fun cleanup(now: Instant) {
        val expiredSessionIds = redisTemplate.opsForZSet()
            .rangeByScore(EXPIRE_INDEX_KEY, 0.0, now.toEpochMilli().toDouble())
            .orEmpty()
        ...
        redisTemplate.opsForHash<String, String>().delete(HASH_KEY, *expiredSessionIds.toTypedArray())
        redisTemplate.opsForZSet().remove(EXPIRE_INDEX_KEY, *expiredSessionIds.toTypedArray())
    }
}
```
...
```shell
HSET active:sessions "1714719600000:session:1" "{...}"
```
...
```shell
HEXPIRE active:sessions 1800 NX FIELDS 1 session:1
HEXPIRE active:sessions 3600 XX FIELDS 1 session:1
HEXPIRE active:sessions 7200 GT FIELDS 1 session:1
HEXPIRE active:sessions 300 LT FIELDS 1 session:1
```
...
class SessionService(
    ...
    fun saveSession(sessionId: String, payload: String) {
        cache.putWithTtl(HASH_KEY, sessionId, payload, Duration.ofMinutes(30))
    }

    fun findSession(sessionId: String): String? {
        return cache.get(HASH_KEY, sessionId)
    }
}
```
...
위 예시는 Redis 7.4에서 `HEXPIRE`를 쓰는 가장 기본적인 형태입니다. 다만 방금 본 두 가지 주의점이 특히 민감한 곳이 세션 캐시입니다. 세션처럼 저장과 TTL 설정이 항상 함께 가야 하는 데이터는 Redis 8의 `HSETEX`까지 같이 보는 편이 자연스럽습니다.

### 3.2 세션 캐시처럼 TTL이 항상 함께 가는 경우
...
```shell
HSET active:sessions session:1 "{...updated...}"
HEXPIRE active:sessions 1800 FIELDS 1 session:1
```
...
```shell
HSETEX active:sessions EX 1800 FIELDS 1 session:1 "{...updated...}"
```

핵심 차이는 중간 상태가 생기느냐입니다. `HSET`과 `HEXPIRE`를 따로 호출하면 둘 사이에 실패 구간이 생깁니다. `HSETEX`는 값 저장과 TTL 설정을 한 번에 처리합니다. 그래서 세션, 인증 토큰, 일회성 코드처럼 반드시 만료되어야 하는 데이터에 더 잘 맞습니다.
...
class Redis8SessionCache(
    ...
    fun put(sessionId: String, payload: String, ttl: Duration) {
        ...
        val changed = hash.putAndExpire(
            HASH_KEY,
            mapOf(sessionId to payload),
            RedisHashCommands.HashFieldSetOption.upsert(),
            Expiration.from(ttl)
        )

        if (changed != true) {
            throw IllegalStateException("세션 저장 실패. sessionId=$sessionId")
        }
    }

    fun getAndRefresh(sessionId: String, ttl: Duration): String? {
        val hash = redisTemplate.opsForHash<String, String>()
...
| 조회할 때마다 TTL을 연장하는 sliding session이다       | `HGETEX`, 즉 `HashOperations.getAndExpire`                 |
...
```shell
HSETEX active:sessions EX 1800 FIELDS 1 session:1 "{...}"
HGETEX active:sessions EX 1800 FIELDS 1 session:1
HGETDEL active:sessions FIELDS 1 session:1
```
...
commands.hset("active:sessions", "session:1", "{...}")
...
`HEXPIRE`는 단일 Hash key 명령이라 Cluster에서도 cross-slot 문제는 없습니다. 즉 명령 자체는 그대로 쓸 수 있습니다. 다만 모든 세션을 하나의 Hash에 몰아넣으면 특정 slot 하나에 쓰기와 만료 처리가 몰려 hot key가 될 수 있습니다.

트래픽이 작으면 하나의 Hash로도 충분합니다.

```text
active:sessions
```

세션 수가 많고 write가 잦다면 tenant나 bucket 단위로 나누는 편이 낫습니다.

```text
active:sessions:{tenant-a:00}
active:sessions:{tenant-a:01}
active:sessions:{tenant-a:02}
active:sessions:{tenant-b:00}
```

Spring 코드에서는 아래처럼 key를 만들 수 있습니다.

```kotlin
class SessionHashKeyResolver {

    fun resolve(tenantId: String, sessionId: String): String {
        val bucket = Math.floorMod(sessionId.hashCode(), BUCKET_SIZE)
            .toString()
            .padStart(2, '0')

        // Redis Cluster hash tag는 중괄호 안의 값으로 slot을 계산한다.
        return "active:sessions:{$tenantId:$bucket}"
    }
...
이렇게 하면 tenant 안에서도 session id 기준으로 bucket이 나뉘어 부하를 분산할 수 있습니다. 반대로 특정 session과 함께 갱신해야 하는 보조 key가 있다면 같은 hash tag를 써서 같은 slot에 묶어야 합니다.

```text
active:sessions:{tenant-a:03}
active:sessions:index:{tenant-a:03}
```
